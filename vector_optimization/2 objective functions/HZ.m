        ...
        while 1
            ...
            while 1
               % Compute the stepsize satisfying the modified strong Wolfe conditions
                [alphak,nf,ng]=qwolfe2(xk,dk,alphak,alphamax,epsiki,nf,ng,d,r1,r2);
                fxkvk=max([r1*g1(xk,d)'*vk,r2*g2(xk,d)'*vk]);
                xki=xk+alphak*dk;
                F1=[r1*f1(xki,d);r2*f2(xki,d)];
                nf=nf+2;
                H=zeros(n+1,n+1);
                H(2:n+1,2:n+1)=eye(n);
                ff=zeros(n+1,1);
                ff(1,1)=1;
                A=[-1 r1*g1(xki,d)' ;-1 r2*g2(xki,d)'];
                ng=ng+2;
                b=zeros(m,1);
                xx=ones(n+1,1)+2;
                ...
                vki=d1(2:n+1);
                fxkivki=d1(1);
                fxkvki=max([r1*g1(xk,d)'*vki,r2*g2(xk,d)'*vki]);
                p=-fxkivki+fxkvki;
                fxkidk=max([r1*g1(xki,d)'*dk,r2*g2(xki,d)'*dk]);
                fxkivk=max([r1*g1(xki,d)'*vk,r2*g2(xki,d)'*vk]);
                q=fxkivk-fxkvk;
                ...
                dki=vki+betaki*dk;
                fxkidki=max([r1*g1(xki,d)'*(dki),r2*g2(xki,d)'*(dki)]);
                if fxkidki<= c*fxkivki
                    break
                elseif betaki<0
                    h=max([r1*g1(xki,d)'*(-dk),r2*g2(xki,d)'*(-dk)]);
            ...
            vk=vki;
            xk=xki;
            fxk1dk1=max([r1*g1(xk,d)'*(dk),r2*g2(xk,d)'*(dk)]);
            % Compute initial step (k>1)

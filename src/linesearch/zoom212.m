...
function [alphak,nf,ng]=zoom212(alpha1,alpha0,epsiki,xk,dk,sigmaba,rhoba,nf,ng,d,r1,r2)
s=0;smax=20;
while (s<smax)
    ...
    if r1*f1(xk+alphaj*dk,d)>r1*f1(xk,d)+alphaj*rhoba*max([r1*g1(xk,d)'*dk,r2*g2(xk,d)'*dk])  ||  r1*f1(xk+alphaj*dk,d)>=r1*f1(xk+alpha1*dk,d)
        ...
        if  (r1*g1(xk+alphaj*dk,d)'*dk>=sigmaba*max([r1*g1(xk,d)'*dk,r2*g2(xk,d)'*dk])&& r1*g1(xk+alphaj*dk,d)'*dk<=epsiki)
            ng=ng+1;
            alphak=alphaj;
            break;
        end

...
% direction_hz  HZ direction using quadratic subproblem (initial step).
%   [d, state] = direction_hz(x, grad, history, opts)
%   Computes v by solving the HZ subproblem and returns d = v.
%   This matches the first-iteration rule (dk = vk) in legacy HZ.m.
%   Advanced update (betaki) will be added when integrating with step updates.
...
d = v;
state = struct('name','hz','fval',fval,'qp_exitflag',out.exitflag);

...
for d=1:12
    for iLoop=1:Loop
        ...
        [d1,sitak1]= quadprog(H,ff,A,b,[],[],[],[],xx);
                ...
                p=-fxk1vk1+fxkvk1;
                fxk1dk=max([r1*g1(x1,d)'*dk,r2*g2(x1,d)'*dk,r3*g3(x1,d)'*dk]);
                fxkdk=max([r1*g1(xk,d)'*dk,r2*g2(xk,d)'*dk,r3*g3(xk,d)'*dk]);
                gamma=max(norm(r1*g1(x1,d)-r1*g1(xk,d))^2,max(norm(r2*g2(x1,d)-r2*g2(xk,d))^2,norm(r3*g3(x1,d)-r3*g3(xk,d))^2));
        ...
        %F3=[r1*f1(xk,d);r2*f2(xk,d);r3*f3(xk,d)];
        F3=[f1(xk,d);f2(xk,d);f3(xk,d)];
        t=toc;
...
%         plot3(F(1,1),F(2,1),F(3,1),'.k',F1(1,1),F1(2,1),F1(3,1),'r*');
%         plot3([F(1,1),F1(1,1)],[F(2,1),F1(2,1)],[F(3,1),F1(3,1)],'k');
%         grid off;
%         %axis([0 15 0 15 0 20])
%         xlabel({'$f_1(x)$'},'Interpreter','latex')
%         ylabel({'$f_2(x)$'},'Interpreter','latex')
%         zlabel({'$f_3(x)$'},'Interpreter','latex')
%         title('MOP7','fontname','Times New Roman','Color','k','FontSize',13)
%         pause(0.1);
%         hold on;
    end
end
